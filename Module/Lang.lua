...
local function get_ietf_parts (source, args_script, args_region, args_variant)
	...
	if not code then
		return nil, nil, nil, nil, nil, table.concat ({'無法識別嘅語言標籤：', source});		-- don't know what we got but it is malformed
	end

	code = code:lower();														-- ensure that we use and return lower case version of this
	
	if not (override_table[code] or lang_table[code] or synonym_table[code] or lang_dep_table[code]) then
		return nil, nil, nil, nil, nil, table.concat ({'無法識別嘅語言代碼：', code});		-- invalid language code, don't know about the others (don't care?)
	end
...
local function make_error_msg (msg, args, template)
	...
	if 'Transl' == template then
		...
		category = 'Lang同lang-xx'
	end
	
	table.insert (out, table.concat ({'[', args.text or 'undefined', '] '}));	-- for error messages output args.text if available
	table.insert (out, table.concat ({'<span style=\"font-size: 100%; font-style: normal;\" class=\"error\">出錯：{{', template, '}}: '}));
	table.insert (out, msg);
	table.insert (out, table.concat ({' ([[:Category:', category, ' template errors|幫手]])'}));
...
local function make_text_html (code, text, tag, rtl, style, size, language)
	...
	if is_set (language) then
		...
		if language:find ('languages') then
			...
			table.insert (html, '文字');								-- for individual languages
		end
...
for Cantonese:
	[[Category:Articles containing explicitly cited Cantonese-language text]]
...
local function make_category (code, language_name, nocat, name_get)
	...
	table.insert (cat, '[[Category:有');

	if 'yue' == code then
		table.insert (cat, '明確引用' .. language_name);				-- falls back to Cantonese if regional name not available
	else
		table.insert (cat, language_name);
	end
	
	table.insert (cat, '嘅文章]]');
	...
	if is_set (args.translation) then											-- translation (not supported in {{lang}})
		...
		if 'none' ~= args.label then
			...
			if 'no' == args.link then
				...
				table.insert (out, make_wikilink ('直譯', '直譯'));
			end
			table.insert (out, "&nbsp;</small>");
		end
